...
var PP_FASES_DEFAULT = ['Fase ofensiva: saque portero','Fase ofensiva: inicios de juego','Fase ofensiva: ataque posicional','Fase ofensiva: fase de finalización','Transición ataque - defensa','Fase defensiva: saque portero rival','Fase defensiva: presión alta','Fase defensiva: bloque medio','Fase defensiva: bloque bajo','Fase defensiva: defensa posicional','Transición defensa - ataque','Otros aspectos a tener en cuenta'].map(function(t,i){return{id:'f'+i,title:t,notes:'',media:[]}});
...
var PP_OUR_SECTIONS = [
    {type:'title',label:'SISTEMA DE JUEGO',color:'#ec4899'},
    {id:'sys_inicial',title:'Sistema inicial',color:'#ec4899'},
    {id:'sys_variantes',title:'Variantes de sistema / Plan B',color:'#ec4899'},
...
async function ppCargarPlantilla(){if(!seasonId)return;try{var{data,error}=await supabaseClient.from('season_players').select('id,shirt_number,players(name,position,photo_url,status,fecha_baja)').eq('season_id',seasonId).order('shirt_number',{ascending:true});if(error)throw error;pp.plantilla=(data||[]).map(function(sp){return{id:String(sp.id),number:sp.shirt_number,name:sp.players?sp.players.name:'?',position:sp.players?sp.players.position:'',photo:sp.players?sp.players.photo_url:'',baja:(sp.players&&sp.players.status==='baja')?(sp.players.fecha_baja||'0000-00-00'):null}});pp.plantillaTodos=pp.plantilla.slice();ppAplicarBajas()}catch(e){pp.plantilla=[]}}
// Deja en pp.plantilla solo los jugadores activos en la fecha del partido seleccionado (o hoy si no hay partido)
function ppAplicarBajas(){var f=(pp.partidoActual&&pp.partidoActual.match_date)||new Date().toISOString().slice(0,10);pp.plantilla=(pp.plantillaTodos||pp.plantilla||[]).filter(function(j){return !(j.baja&&f>=j.baja)})}

// === VISOR ===
function ppMostrarVisor(src,texto){var prev=document.getElementById('pp-img-viewer');if(prev)prev.remove();var ov=document.createElement('div');ov.id='pp-img-viewer';ov.style.cssText='position:fixed;inset:0;background:rgba(0,0,0,.9);z-index:999999;display:flex;flex-direction:column;align-items:center;justify-content:center;gap:14px;padding:20px;cursor:pointer';ov.onclick=function(e){if(e.target===ov||e.target.id==='pp-visor-x')ov.remove()};var h='<img src="'+src+'" style="max-width:92vw;max-height:'+(texto?'58vh':'90vh')+';border-radius:8px;box-shadow:0 4px 20px rgba(0,0,0,0.5)">';if(texto)h+='<div style="background:rgba(15,23,42,.96);border:1px solid #334155;border-radius:12px;padding:16px 22px;color:#e2e8f0;font-size:20px;line-height:1.6;max-width:920px;width:92vw;max-height:28vh;overflow-y:auto;white-space:pre-wrap;cursor:auto">'+ppEsc(texto)+'</div>';h+='<div id="pp-visor-x" style="position:absolute;top:16px;right:20px;color:#fff;font-size:26px;cursor:pointer">\u2715</div>';ov.innerHTML=h;document.body.appendChild(ov)}
...
function ppMostrarTab(t){['scouting','jugadores','fases','tactica','confront','abp','semana'].forEach(function(x){var b=document.getElementById('pp-tab-'+x);if(b){var a=x===t;b.style.borderColor=a?'#3b82f6':'#334155';b.style.background=a?'#1e3a5f':'#0f172a';b.style.color=a?'#93c5fd':'#9ca3af'}});var area=document.getElementById('pp-tab-content');if(!area)return;if(t==='scouting')area.innerHTML=ppRenderScouting();else if(t==='jugadores')area.innerHTML=ppRenderJugadores();else if(t==='fases')area.innerHTML=ppRenderFases();else if(t==='tactica')area.innerHTML=ppRenderTactica();else if(t==='confront')area.innerHTML=ppRenderConfront();else if(t==='confront')area.innerHTML=ppRenderConfront();else if(t==='abp'){area.innerHTML=ppRenderABPs();if(!pp._abpSync){pp._abpSync=true;ppAbpSincronizarPizarra().finally(function(){pp._abpSync=false})}}else if(t==='semana'){area.innerHTML='<div style="text-align:center;padding:30px;color:#64748b">Cargando...</div>';ppCargarSemana()}else if(t==='contenido')area.innerHTML=ppRenderContenidos()}
...
// === TAB: FASES RIVAL ===
function ppGetFases(){var f=pp.planActual.tactical_phases;if(f&&Array.isArray(f)&&f.length)return f;var cd=clubData&&Array.isArray(clubData.pp_fases_default)&&clubData.pp_fases_default.length?clubData.pp_fases_default:null;if(cd)return cd.map(function(t,i){return{id:'c'+i,title:String(t),notes:'',media:[]}});return PP_FASES_DEFAULT.map(function(x){return{id:x.id,title:x.title,notes:'',media:[]}})}
function ppRenderFases(){var fases=ppGetFases();var cols=['#0f6e56','#085041','#993c1d','#0c447c','#3c3489','#712b13'];var h='<div style="background:#0f172a;border:1px solid #1e3a5f;border-radius:12px;padding:20px"><h3 style="margin:0 0 6px;color:#e2e8f0;font-size:16px">⚽ Fases del juego rival</h3><p style="margin:0 0 16px;font-size:12px;color:#64748b">Analiza cada fase. Puedes renombrar, añadir o quitar fases segun tu modelo.</p><div style="display:flex;gap:8px;flex-wrap:wrap;margin-bottom:14px"><button onclick="ppAnadirFase()" style="padding:6px 14px;background:#3b82f6;border:none;color:#fff;border-radius:8px;cursor:pointer;font-size:12px;font-weight:600">+ Añadir fase</button><button onclick="ppGuardarFasesDefecto()" style="padding:6px 14px;background:#1e293b;border:1px solid #334155;color:#94a3b8;border-radius:8px;cursor:pointer;font-size:12px" title="Los nuevos planes de partido usaran estas fases">💾 Usar como mis fases por defecto</button>'+(clubData&&clubData.pp_fases_default?'<button onclick="ppRestaurarFasesDefecto()" style="padding:6px 14px;background:none;border:1px solid #334155;color:#64748b;border-radius:8px;cursor:pointer;font-size:12px">↺ Volver a la lista estandar</button>':'')+'</div>';fases.forEach(function(f,i){var mh='';if(f.media&&f.media.length){mh='<div style="display:flex;flex-wrap:wrap;gap:8px;margin-top:8px">';f.media.forEach(function(m,mi){if(m.type==='image'&&m.url){mh+='<div style="display:inline-flex;flex-direction:column;align-items:center;background:#0f172a;border:1px solid #334155;border-radius:8px;padding:6px;position:relative;max-width:160px"><img src="'+m.url+'" onclick="ppVerImagenFase('+i+','+mi+')" style="width:150px;height:auto;border-radius:6px;cursor:pointer"><div style="font-size:10px;margin-top:4px;color:#94a3b8;max-width:140px;overflow:hidden;text-overflow:ellipsis;white-space:nowrap">'+ppEsc(m.title||'')+'</div><button onclick="ppEliminarMediaFase('+i+','+mi+')" style="position:absolute;top:2px;right:2px;background:#7f1d1d;border:none;color:#fca5a5;cursor:pointer;font-size:10px;width:18px;height:18px;border-radius:50%;display:flex;align-items:center;justify-content:center">✕</button></div>'}else{mh+='<div style="display:flex;align-items:center;gap:6px;background:#0f172a;border:1px solid #334155;border-radius:6px;padding:5px 10px;font-size:11px"><span style="font-size:14px">'+(m.type==='video'?'🎬':'🔗')+'</span><a href="'+ppEsc(m.url)+'" target="_blank" style="color:#60a5fa;text-decoration:none;max-width:200px;overflow:hidden;text-overflow:ellipsis;white-space:nowrap">'+ppEsc(m.title||'Media')+'</a><button onclick="ppEliminarMediaFase('+i+','+mi+')" style="background:none;border:none;color:#ef4444;cursor:pointer;font-size:14px">✕</button></div>'}});mh+='</div>'}h+='<div style="background:#1e293b;border-left:4px solid '+cols[i%cols.length]+';border-radius:0 8px 8px 0;padding:14px;margin-bottom:10px"><div style="display:flex;align-items:center;justify-content:space-between;margin-bottom:8px"><div style="display:flex;align-items:center;gap:6px;min-width:0"><div style="font-size:13px;font-weight:600;color:#e2e8f0">'+ppEsc(f.title)+'</div><button onclick="ppRenombrarFase('+i+')" title="Renombrar" style="background:none;border:none;color:#64748b;cursor:pointer;font-size:12px;padding:2px">✏️</button><button onclick="ppEliminarFase('+i+')" title="Eliminar fase" style="background:none;border:none;color:#64748b;cursor:pointer;font-size:12px;padding:2px">🗑️</button></div><button onclick="ppAgregarMediaFase('+i+')" style="padding:4px 10px;background:#0f172a;border:1px solid #334155;color:#94a3b8;border-radius:6px;cursor:pointer;font-size:11px">+ Video/Imagen</button></div><textarea id="pp-fase-'+i+'" onchange="ppGuardarFase('+i+',this.value)" rows="3" placeholder="Notas..." style="width:100%;padding:8px 10px;background:#0f172a;border:1px solid #334155;border-radius:6px;color:#e2e8f0;font-size:13px;resize:vertical">'+ppEsc(f.notes)+'</textarea>'+mh+'</div>'});return h+'</div>'}
...
async function ppAbpSubirImagen(input){var file=input.files[0];if(!file)return;try{var dataUrl=await ppComprimirImagen(file,1400);_ppAbpForm.imagen=dataUrl;_ppAbpForm.pizarra_item_id=null;_ppAbpForm.pizarra_nombre='';ppCollectAbpFormData();ppRenderAbpForm()}catch(e){showToast('Error: '+e.message)}}
function ppAbpQuitarImagen(){_ppAbpForm.imagen='';_ppAbpForm.pizarra_item_id=null;_ppAbpForm.pizarra_nombre='';ppCollectAbpFormData();ppRenderAbpForm()}

// === ABP desde pizarra (proyectos) ===
// Convierte la miniatura del item (data URL o SVG en texto) a PNG data URL
...
async function ppAbpAbrirPizarra(){
    ppCollectAbpFormData();
    var prev=document.getElementById('pp-piz-overlay');if(prev)prev.remove();
    var ov=document.createElement('div');ov.id='pp-piz-overlay';
    ...
    ov.onclick=function(e){if(e.target===ov)ov.remove()};
    ov.innerHTML='<div style="background:#0f172a;border:1px solid #1e3a5f;border-radius:14px;max-width:760px;width:100%;max-height:85vh;display:flex;flex-direction:column"><div style="padding:14px 18px;border-bottom:1px solid #1e3a5f;display:flex;justify-content:space-between;align-items:center"><h3 style="margin:0;color:#c4b5fd;font-size:15px">🎨 Elegir fase de un proyecto de pizarra</h3><button onclick="document.getElementById(\'pp-piz-overlay\').remove()" style="background:none;border:none;color:#9ca3af;font-size:20px;cursor:pointer">×</button></div><div id="pp-piz-body" style="padding:16px;overflow-y:auto;color:#94a3b8;font-size:13px">Cargando proyectos...</div></div>';
    document.body.appendChild(ov);
    try{
        var cId=window.clubId||clubId;
        var rp=await supabaseClient.from('pizarra_proyectos').select('id, nombre').or('club_id.eq.'+cId+',coach_id.eq.'+String(usuario.id)).order('updated_at',{ascending:false});
        if(rp.error)throw rp.error;
        var proys=rp.data||[];
        if(!proys.length){document.getElementById('pp-piz-body').innerHTML='No tienes proyectos de pizarra. Crea uno en Ejercicios → Pizarra → Proyectos.';return}
        var ri=await supabaseClient.from('pizarra_proyecto_items').select('id, proyecto_id, orden, nombre, thumbnail_svg').in('proyecto_id',proys.map(function(p){return p.id})).order('orden');
        if(ri.error)throw ri.error;
        var items=ri.data||[];
        var h='';
        proys.forEach(function(p){
            var its=items.filter(function(i){return i.proyecto_id===p.id});
            if(!its.length)return;
            h+='<div style="margin-bottom:16px"><div style="font-size:13px;font-weight:700;color:#e2e8f0;margin-bottom:8px">📁 '+ppEsc(p.nombre)+'</div><div style="display:flex;flex-wrap:wrap;gap:10px">';
            its.forEach(function(it,k){
                var src=it.thumbnail_svg?(it.thumbnail_svg.trim().indexOf('data:')===0?it.thumbnail_svg.trim():'data:image/svg+xml;base64,'+btoa(unescape(encodeURIComponent(it.thumbnail_svg)))):'';
                h+='<div onclick="ppAbpElegirPizarra(\''+it.id+'\',\''+ppEsc(p.nombre).replace(/'/g,'')+' / '+ppEsc(it.nombre||('Fase '+(k+1))).replace(/'/g,'')+'\')" style="width:150px;cursor:pointer;background:#1e293b;border:1px solid #334155;border-radius:8px;padding:6px;text-align:center" onmouseenter="this.style.borderColor=\'#7c3aed\'" onmouseleave="this.style.borderColor=\'#334155\'">'+(src?'<img src="'+src+'" style="width:100%;aspect-ratio:8/5;object-fit:cover;border-radius:4px;background:#0f4c2a">':'<div style="width:100%;aspect-ratio:8/5;background:#0f4c2a;border-radius:4px"></div>')+'<div style="font-size:11px;color:#94a3b8;margin-top:4px;overflow:hidden;text-overflow:ellipsis;white-space:nowrap">'+(k+1)+'. '+ppEsc(it.nombre||'Fase '+(k+1))+'</div></div>';
            });
            h+='</div></div>';
        });
        document.getElementById('pp-piz-body').innerHTML=h||'Tus proyectos no tienen fases guardadas todavia.';
    }catch(e){document.getElementById('pp-piz-body').innerHTML='Error: '+e.message}
}
async function ppAbpElegirPizarra(itemId,nombre){
    var ov=document.getElementById('pp-piz-overlay');if(ov)ov.remove();
    try{
        var r=await supabaseClient.from('pizarra_proyecto_items').select('id, thumbnail_svg').eq('id',itemId).single();
...
// Al abrir la pestana ABPs: actualiza las imagenes de las ABPs vinculadas si la pizarra cambio
async function ppAbpSincronizarPizarra(){
    var cards=ppGetAbpCards();
    var vinc=cards.filter(function(c){return c.pizarra_item_id});
    if(!vinc.length)return;
    try{
        var r=await supabaseClient.from('pizarra_proyecto_items').select('id, thumbnail_svg').in('id',vinc.map(function(c){return c.pizarra_item_id}));
...
function ppAbpAgregarVideo(){ppAbrirModalMedia('pp-abp-video-overlay',function(url,title,tipo){ppCollectAbpFormData();_ppAbpForm.video=url;var hv=document.getElementById('ppabp-video');if(hv)hv.value=url;ppRenderAbpForm();showToast('Video añadido')})}
...
function ppPresentacion(){
    if(!pp.planActual||!pp.partidoActual)return;
    var plan=pp.planActual,par=pp.partidoActual,sl=[];
    var rN=par.opponent;
    var pN=(typeof clubData!=='undefined'&&clubData&&clubData.name)?clubData.name:'Nuestro equipo';
    var rLogo=(pp.rivalActual&&pp.rivalActual.logo_url)?pp.rivalActual.logo_url:'';
    var pLogo=(typeof clubData!=='undefined'&&clubData&&clubData.logo_url)?clubData.logo_url:'';
    var eR=rLogo?'<img src="'+rLogo+'" style="width:120px;height:120px;object-fit:contain">':'<div style="width:120px;height:120px;background:#1e293b;border-radius:16px;display:flex;align-items:center;justify-content:center;font-size:60px">\ud83d\udee1\ufe0f</div>';
    var eP=pLogo?'<img src="'+pLogo+'" style="width:120px;height:120px;object-fit:contain">':'';
    function push(html,team,notes){sl.push({html:html,team:team||'',notes:notes||''})}
    function mediaHtml(media){
        var imgs='';
        if(media&&media.length){
            var ii=media.filter(function(m){return (m.type==='image'||m.type==='video'||m.type==='link')&&m.url});
            if(ii.length){
                imgs='<div style="display:flex;flex-wrap:wrap;gap:14px;margin-top:20px;justify-content:center">';
                ii.forEach(function(m){
                    if(m.type==='image'){imgs+='<div style="text-align:center"><img src="'+m.url+'" onclick="ppPresVerImg(this.src)" style="max-width:min(560px,46vw);max-height:44vh;cursor:pointer;border-radius:8px;border:2px solid rgba(255,255,255,0.2)"><div style="font-size:12px;color:#94a3b8;margin-top:6px">'+ppEsc(m.title||'')+'</div></div>'}
                    else{imgs+='<div style="text-align:center"><a href="'+ppEsc(m.url)+'" target="_blank" style="display:inline-flex;align-items:center;gap:8px;padding:10px 20px;background:#1e293b;border:2px solid #f97316;border-radius:10px;color:#f97316;text-decoration:none;font-size:15px;font-weight:600;cursor:pointer">\ud83c\udfac '+ppEsc(m.title||'Ver video')+'</a></div>'}
                });
                imgs+='</div>';
            }
        }
    ...
    var fe=new Date(par.match_date+'T12:00:00').toLocaleDateString('es-ES',{weekday:'long',day:'numeric',month:'long'});
    push('<div style="display:flex;flex-direction:column;align-items:center;justify-content:center;height:100%;gap:24px">'+eR+'<div style="font-size:48px;font-weight:800;color:#fff;text-transform:uppercase">'+ppEsc(rN)+'</div><div style="font-size:24px;color:#f59e0b">'+(par.competition||'')+'</div><div style="font-size:20px;color:#94a3b8">'+fe+'</div></div>','');
    if(plan.rival_formation){
        var sP=ppParseFormacion(plan.rival_formation);
        if(sP)push('<div style="display:flex;flex-direction:column;align-items:center;justify-content:center;height:100%;padding:20px"><div style="font-size:28px;color:#fff;font-weight:700;margin-bottom:16px">'+ppEsc(rN)+' \u2014 '+plan.rival_formation+'</div>'+ppRenderCampo(sP,(plan.weekly_map&&plan.weekly_map.rival_lineup)||{},plan.rival_players||[],'big')+'</div>','rival');
    }
    if(plan.rival_strengths||plan.rival_weaknesses){
        push('<div style="padding:34px 44px;overflow-y:auto;height:100%"><h2 style="font-size:36px;color:#fff;margin:0 0 30px;border-bottom:3px solid #f59e0b;padding-bottom:10px">Scouting</h2><div style="display:grid;grid-template-columns:1fr 1fr;gap:30px">'+(plan.rival_strengths?'<div><div style="font-size:20px;color:#22c55e;font-weight:700;margin-bottom:10px">\ud83d\udcaa Fuertes</div><div style="font-size:20px;color:#e2e8f0;line-height:1.7;white-space:pre-wrap">'+ppEsc(plan.rival_strengths)+'</div></div>':'')+(plan.rival_weaknesses?'<div><div style="font-size:20px;color:#ef4444;font-weight:700;margin-bottom:10px">\ud83d\udcc9 Debiles</div><div style="font-size:20px;color:#e2e8f0;line-height:1.7;white-space:pre-wrap">'+ppEsc(plan.rival_weaknesses)+'</div></div>':'')+'</div></div>','rival');
    }
    (plan.tactical_phases||[]).forEach(function(f){
        if(!f.notes&&(!f.media||!f.media.length))return;
        push('<div style="padding:30px 44px;overflow-y:auto;height:100%"><h2 style="font-size:34px;color:#fff;margin:0 0 22px;border-bottom:3px solid #f59e0b;padding-bottom:10px">'+ppEsc(f.title)+'</h2>'+(f.notes?'<div style="font-size:21px;color:#e2e8f0;line-height:1.7;white-space:pre-wrap">'+ppEsc(f.notes)+'</div>':'')+mediaHtml(f.media)+'</div>','rival',f.notes||'');
    });
    var ourPhases2=ppGetOurPhases();var currentTitle='';
    PP_OUR_SECTIONS.forEach(function(sec){
        if(sec.type==='title'){currentTitle=sec.label;return}
        var d=ourPhases2[sec.id];
        if(!d||(!d.notes&&(!d.media||!d.media.length)))return;
        push('<div style="padding:30px 44px;overflow-y:auto;height:100%"><div style="font-size:15px;color:'+sec.color+';text-transform:uppercase;letter-spacing:1px;margin-bottom:8px">'+currentTitle+'</div><h2 style="font-size:34px;color:#fff;margin:0 0 22px;border-bottom:3px solid '+sec.color+';padding-bottom:10px">'+ppEsc(sec.title)+'</h2>'+(d.notes?'<div style="font-size:21px;color:#e2e8f0;line-height:1.7;white-space:pre-wrap">'+ppEsc(d.notes)+'</div>':'')+mediaHtml(d.media)+'</div>','own',d.notes||'');
    });
    var abpCards=ppGetAbpCards();
    if(abpCards.length){
        push('<div style="display:flex;flex-direction:column;align-items:center;justify-content:center;height:100%"><h2 style="font-size:42px;color:#a855f7;font-weight:800;margin:0">\ud83c\udfaf ABPs</h2><div style="font-size:20px;color:#94a3b8;margin-top:10px">'+abpCards.length+' accion'+(abpCards.length>1?'es':'')+' a balon parado</div></div>','own');
        abpCards.forEach(function(c){
            var tags=[c.tipo,c.subtipo,c.espacio,c.perfil].filter(Boolean).join(' \u00b7 ');
            var color=c.tipo==='ofensiva'?'#22c55e':'#3b82f6';
            var imgHtml=c.imagen&&c.imagen.length>50?'<div style="margin:16px auto;text-align:center"><img src="'+c.imagen+'" onclick="ppPresVerImg(this.src)" style="max-width:min(760px,62vw);max-height:46vh;border-radius:12px;border:3px solid '+color+';cursor:pointer"></div>':'';
            var rolesHtml='';
            if(c.tipo==='ofensiva'){var rols=[];PP_ABP_ROLES_OF.forEach(function(r){var pls=c['of_'+r]||[];if(pls.length)rols.push('<span style="color:'+color+';font-weight:700">'+PP_ABP_ROLES_OF_LABELS[r]+':</span> '+pls.map(function(pid){return ppAbpPlayerName(pid,pp.plantilla)}).join(', '))});if(c.sena)rols.push('<span style="color:#a855f7;font-weight:700">Se\u00f1a:</span> '+ppEsc(c.sena));if(rols.length)rolesHtml='<div style="font-size:17px;color:#e2e8f0;line-height:2;margin-top:16px">'+rols.join('<br>')+'</div>'}
            if(c.tipo==='defensiva'){var rols=[];(c.marcas||[]).forEach(function(m,i){if(m.rival_idx||m.propio_id){var rn=ppAbpPlayerName(m.rival_idx,(pp.planActual.rival_players||[]).map(function(j,k){return{id:'r_'+k,number:j.number,name:j.name}}));var pn=ppAbpPlayerName(m.propio_id,pp.plantilla);rols.push('<span style="color:#f59e0b">Marca '+(i+1)+':</span> '+rn+' \u2694\ufe0f '+pn)}});PP_ABP_ROLES_DF.forEach(function(r){var pls=c['df_'+r]||[];if(pls.length)rols.push('<span style="color:'+color+';font-weight:700">'+PP_ABP_ROLES_DF_LABELS[r]+':</span> '+pls.map(function(pid){return ppAbpPlayerName(pid,pp.plantilla)}).join(', '))});if(rols.length)rolesHtml='<div style="font-size:16px;color:#e2e8f0;line-height:2;margin-top:16px">'+rols.join('<br>')+'</div>'}
            push('<div style="padding:26px 44px;overflow-y:auto;height:100%"><div style="display:flex;align-items:center;gap:12px;margin-bottom:8px"><div style="font-size:15px;color:'+color+';text-transform:uppercase;letter-spacing:1px;font-weight:700">'+ppEsc(c.tipo)+'</div><div style="font-size:13px;color:#64748b">'+ppEsc(tags)+'</div></div><h2 style="font-size:34px;color:#fff;margin:0 0 12px;border-bottom:3px solid '+color+';padding-bottom:10px">'+ppEsc(c.name||'ABP')+'</h2>'+imgHtml+rolesHtml+(c.explicacion?'<div style="font-size:18px;color:#cbd5e1;margin-top:16px;line-height:1.6;white-space:pre-wrap">'+ppEsc(c.explicacion)+'</div>':'')+'</div>','own',c.explicacion||'');
        });
    }
    (plan.linked_exercises||[]).forEach(function(ej){if(ej.thumbnail_svg)push('<div style="display:flex;flex-direction:column;align-items:center;justify-content:center;height:100%;padding:40px"><div style="font-size:24px;color:#fff;font-weight:700;margin-bottom:16px">'+ppEsc(ej.name)+'</div><div style="width:80%;max-width:900px;aspect-ratio:8/5;border-radius:12px;overflow:hidden;background:#0f4c2a">'+ej.thumbnail_svg+'</div></div>','own')});
    push('<div style="display:flex;flex-direction:column;align-items:center;justify-content:center;height:100%;gap:20px">'+eP+'<div style="font-size:36px;font-weight:800;color:#f59e0b">VAMOS A POR ELLOS</div></div>','');
    if(!sl.length){showToast('Sin contenido');return}
    window._ppPresSlides=sl;window._ppPresIdx=0;
    function hdr(team){
        if(team==='rival')return '<div style="flex-shrink:0;display:flex;align-items:center;gap:12px;padding:9px 22px;background:rgba(127,29,29,.35);border-bottom:2px solid #ef4444">'+(rLogo?'<img src="'+rLogo+'" style="width:36px;height:36px;object-fit:contain">':'<span style="font-size:22px">\ud83d\udee1\ufe0f</span>')+'<span style="font-size:19px;font-weight:800;color:#fff;text-transform:uppercase;letter-spacing:.5px">'+ppEsc(rN)+'</span><span style="margin-left:auto;font-size:12px;font-weight:800;color:#fca5a5;background:#7f1d1d;border:1px solid #ef4444;padding:4px 12px;border-radius:20px;letter-spacing:1px">AN\u00c1LISIS RIVAL</span></div>';
        if(team==='own')return '<div style="flex-shrink:0;display:flex;align-items:center;gap:12px;padding:9px 22px;background:rgba(6,95,70,.35);border-bottom:2px solid #10b981">'+(pLogo?'<img src="'+pLogo+'" style="width:36px;height:36px;object-fit:contain">':'<span style="font-size:22px">\u26bd</span>')+'<span style="font-size:19px;font-weight:800;color:#fff;text-transform:uppercase;letter-spacing:.5px">'+ppEsc(pN)+'</span><span style="margin-left:auto;font-size:12px;font-weight:800;color:#6ee7b7;background:#065f46;border:1px solid #10b981;padding:4px 12px;border-radius:20px;letter-spacing:1px">NUESTRO PLAN</span></div>';
        return '';
    }
    function render(){
        var s=sl[window._ppPresIdx];
        var c=document.getElementById('pp-pres-content');
        if(c)c.innerHTML=hdr(s.team)+'<div style="flex:1;min-height:0;position:relative">'+s.html+'</div>';
        var ct=document.getElementById('pp-pres-counter');
        if(ct)ct.textContent=(window._ppPresIdx+1)+' / '+sl.length;
    }
    window._ppPresRender=render;
    var ov=document.createElement('div');
    ov.id='pp-presentacion';
    ov.style.cssText='position:fixed;inset:0;z-index:99999;background:#0a0a0a;display:flex;flex-direction:column';
    ov.innerHTML='<div id="pp-pres-content" style="flex:1;min-height:0;display:flex;flex-direction:column"></div><div style="flex-shrink:0;padding:8px 16px;display:flex;align-items:center;justify-content:space-between;background:#111;border-top:1px solid #1e293b"><button onclick="ppPresAnterior()" style="padding:7px 18px;background:#1e293b;border:1px solid #334155;color:#94a3b8;border-radius:6px;cursor:pointer;font-size:14px">\u2190 Anterior</button><span id="pp-pres-counter" style="color:#64748b;font-size:14px"></span><div style="display:flex;gap:8px"><button onclick="ppPresSiguiente()" style="padding:7px 18px;background:#3b82f6;border:none;color:#fff;border-radius:6px;cursor:pointer;font-size:14px">Siguiente \u2192</button><button onclick="ppPresCerrar()" style="padding:7px 14px;background:#7f1d1d;border:1px solid #dc2626;color:#fca5a5;border-radius:6px;cursor:pointer;font-size:14px">\u2715 Salir</button></div></div>';
    document.body.appendChild(ov);
    render();
    try{if(ov.requestFullscreen){var fp=ov.requestFullscreen();if(fp&&fp.catch)fp.catch(function(){})}}catch(e){}
    document.addEventListener('keydown',ppPresKeyHandler);
}
function ppPresVerImg(src){var s=window._ppPresSlides&&window._ppPresSlides[window._ppPresIdx];ppMostrarVisor(src,s?s.notes:'')}
...
function ppPresCerrar(){var el=document.getElementById('pp-presentacion');if(el)el.remove();try{if(document.fullscreenElement)document.exitFullscreen()}catch(e){}document.removeEventListener('keydown',ppPresKeyHandler)}
function ppPresKeyHandler(e){var visor=document.getElementById('pp-img-viewer');if(e.key==='Escape'&&visor){visor.remove();return}if(e.key==='ArrowRight'||e.key===' '){e.preventDefault();ppPresSiguiente()}else if(e.key==='ArrowLeft'){e.preventDefault();ppPresAnterior()}else if(e.key==='Escape'){ppPresCerrar()}}
...
// ===== FASE 3: Importar plantilla rival al plan de partido =====
async function ppImportarPlantillaRival(){
    if(!pp.planActual){showToast('Carga primero un plan de partido');return}
    try{
        var cId=window.clubId||clubId;
        var res=await supabaseClient.from('equipos_rivales').select('id, nombre').eq('club_id',cId).order('nombre');
        if(res.error)throw res.error;
        var equipos=res.data||[];
        if(!equipos.length){showToast('No tienes plantillas rivales. Créalas en Gestión de Competición → Rivales.');return}
        var rivalNombre=(pp.partidoActual&&pp.partidoActual.opponent?pp.partidoActual.opponent:'').trim().toLowerCase();
        var preId=equipos[0].id;
        equipos.forEach(function(e){
            var n=(e.nombre||'').trim().toLowerCase();
            if(rivalNombre&&(n===rivalNombre||n.indexOf(rivalNombre)>-1||rivalNombre.indexOf(n)>-1))preId=e.id;
        });
        var prev=document.getElementById('pp-import-ov');if(prev)prev.remove();
        var ov=document.createElement('div');ov.id='pp-import-ov';
        ov.style.cssText='position:fixed;inset:0;background:rgba(0,0,0,.7);z-index:9999;display:flex;align-items:center;justify-content:center;padding:16px';
        ov.onclick=function(e){if(e.target===ov)ov.remove()};
        var opc='';equipos.forEach(function(e){opc+='<option value="'+e.id+'"'+(e.id===preId?' selected':'')+'>'+ppEsc(e.nombre)+'</option>'});
        var hay=(pp.planActual.rival_players||[]).length;
        ov.innerHTML='<div style="background:#0f172a;border:1px solid #1e3a5f;border-radius:12px;max-width:420px;width:100%;padding:22px">'
            +'<h3 style="margin:0 0 6px;color:#e2e8f0;font-size:16px">📥 Importar de plantilla rival</h3>'
            +'<p style="margin:0 0 14px;font-size:12px;color:#64748b">Copia al plan los jugadores con toda su ficha (foto, pie, stats, análisis del analista). Después puedes retocarlos sin afectar a la plantilla original.</p>'
            +'<label style="font-size:11px;color:#9ca3af;display:block;margin-bottom:4px">Equipo rival</label>'
            +'<select id="pp-import-equipo" style="width:100%;padding:8px 10px;background:#1e293b;border:1px solid #334155;border-radius:6px;color:#e2e8f0;font-size:13px;margin-bottom:14px">'+opc+'</select>'
            +(hay?'<label style="font-size:11px;color:#9ca3af;display:block;margin-bottom:6px">Ya hay '+hay+' jugador(es) en el plan</label>'
            +'<div style="display:flex;gap:14px;margin-bottom:16px;font-size:13px;color:#e2e8f0">'
            +'<label style="display:flex;align-items:center;gap:6px;cursor:pointer"><input type="radio" name="pp-import-modo" value="add" checked> Añadir a los actuales</label>'
            +'<label style="display:flex;align-items:center;gap:6px;cursor:pointer"><input type="radio" name="pp-import-modo" value="replace"> Reemplazar lista</label>'
            +'</div>':'')
            +'<div style="display:flex;gap:8px;justify-content:flex-end">'
            +'<button onclick="document.getElementById(\'pp-import-ov\').remove()" style="padding:8px 16px;background:#1e293b;border:1px solid #475569;color:#9ca3af;border-radius:6px;cursor:pointer;font-size:13px">Cancelar</button>'
            +'<button onclick="ppEjecutarImportRival()" style="padding:8px 16px;background:#16a34a;border:none;color:#fff;border-radius:6px;cursor:pointer;font-size:13px;font-weight:600">Importar</button>'
            +'</div></div>';
        document.body.appendChild(ov);
    }catch(e){showToast('Error: '+e.message)}
}
async function ppEjecutarImportRival(){
    var sel=document.getElementById('pp-import-equipo');if(!sel)return;
    var equipoId=sel.value;
    var modoEl=document.querySelector('input[name="pp-import-modo"]:checked');
    var modo=modoEl?modoEl.value:'add';
    try{
        var res=await supabaseClient.from('jugadores_rivales').select('*').eq('equipo_rival_id',equipoId).order('dorsal');
        if(res.error)throw res.error;
        var origen=res.data||[];
        if(!origen.length){showToast('Ese equipo no tiene jugadores en su plantilla');return}
        function tipoDe(url){if(!url)return 'link';if(url.match(/\.(mp4|mov|webm)$/i)||url.indexOf('youtube')>=0||url.indexOf('youtu.be')>=0||url.indexOf('veo.co')>=0)return 'video';if(url.match(/\.(jpg|jpeg|png|gif|webp)$/i))return 'image';return 'link'}
        var nuevos=origen.map(function(j){return{
            name:j.nombre||'',
            number:j.dorsal?(parseInt(j.dorsal,10)||null):null,
            position:j.posicion||'',
            foot:j.pie||'Diestro',
            year:j.anio?(parseInt(j.anio,10)||null):null,
            club_from:j.procede||'',
            games:(j.pj!=null)?j.pj:null,
            minutes:(j.minutos!=null)?j.minutos:null,
            goals:(j.goles!=null)?j.goles:null,
            analysis:j.analisis||'',
            photo:j.foto_url||'',
            media:(j.archivos||[]).map(function(a){return{url:a.url,title:a.titulo||'',type:tipoDe(a.url)}})
        }});
        var jug=(modo==='replace')?[]:(pp.planActual.rival_players||[]).slice();
        var existentes={};jug.forEach(function(j){existentes[(j.name||'').trim().toLowerCase()]=true});
        var anadidos=0,saltados=0;
        nuevos.forEach(function(n){
            var k=(n.name||'').trim().toLowerCase();
            if(existentes[k]){saltados++;return}
            jug.push(n);existentes[k]=true;anadidos++;
        });
        await supabaseClient.from('match_plans').update({rival_players:jug,updated_at:new Date().toISOString()}).eq('id',pp.planActual.id);
        pp.planActual.rival_players=jug;
        var ov=document.getElementById('pp-import-ov');if(ov)ov.remove();
        showToast('Importados '+anadidos+' jugador(es)'+(saltados?' · '+saltados+' ya estaban en el plan':''));
        ppMostrarTab('jugadores');
    }catch(e){showToast('Error al importar: '+e.message)}
}
// =============================================
// VIGILANCIA: jugadores propios implicados en cada rival + Fichas vestuario (PDF)
// =============================================
var _ppJugadorVig=[];
function ppRenderVigilantesPicker(sel){sel=sel||[];var pl=pp.plantilla||[];var h='<div style="margin-bottom:12px"><label style="font-size:11px;color:#9ca3af;display:block;margin-bottom:3px">👁 Vigilancia — jugadores propios implicados</label>';if(!pl.length)return h+'<div style="font-size:11px;color:#475569">Sin plantilla cargada en la temporada</div></div>';h+='<div id="ppj-vig-chips" style="display:flex;flex-wrap:wrap;gap:6px">';pl.forEach(function(p){var on=sel.indexOf(p.id)>=0;h+='<span onclick="ppToggleVigilante(\''+p.id+'\')" style="cursor:pointer;user-select:none;padding:4px 10px;border-radius:14px;font-size:11px;border:1px solid '+(on?'#f59e0b':'#334155')+';background:'+(on?'#78350f':'#1e293b')+';color:'+(on?'#fde68a':'#94a3b8')+'">'+(p.number?p.number+' ':'')+ppEsc(p.name)+'</span>'});return h+'</div></div>'}
function ppToggleVigilante(id){var i=_ppJugadorVig.indexOf(id);if(i>=0)_ppJugadorVig.splice(i,1);else _ppJugadorVig.push(id);var c=document.getElementById('ppj-vig-chips');if(!c)return;var tmp=document.createElement('div');tmp.innerHTML=ppRenderVigilantesPicker(_ppJugadorVig);var nu=tmp.querySelector('#ppj-vig-chips');if(nu)c.outerHTML=nu.outerHTML}
function ppVigilantesInfo(ids){var pl=pp.plantilla||[];var out=[];(ids||[]).forEach(function(id){for(var i=0;i<pl.length;i++){if(pl[i].id===id){out.push(pl[i]);break}}});return out}
function ppRenderVigilantesLinea(ids){var v=ppVigilantesInfo(ids);if(!v.length)return '';return '<div style="font-size:10px;color:#fbbf24;margin-top:4px">👁 Vigilan: '+ppEsc(v.map(function(p){return (p.number?p.number+' ':'')+p.name}).join(' · '))+'</div>'}
async function ppExportarFichasVestuario(){
    if(!pp.planActual||!pp.partidoActual){showToast('Selecciona un partido primero');return}
    var jug=(pp.planActual.rival_players||[]).filter(function(j){return (j.analysis&&j.analysis.trim())||(j.vigilantes&&j.vigilantes.length)});
    if(!jug.length){showToast('Ningun jugador rival tiene analisis o vigilancia asignada');return}
    showToast('Generando fichas...','success');
    var par=pp.partidoActual;
    var rival=par.opponent||'Rival';
    var miEq=(clubData&&clubData.name)||'Mi Equipo';
    var fecha=new Date(par.match_date+'T12:00:00').toLocaleDateString('es-ES',{weekday:'long',day:'numeric',month:'long',year:'numeric'});
    var logoRival=null;try{logoRival=await ppLoadImg((pp.rivalActual&&pp.rivalActual.logo_url)||par.opponent_logo)}catch(e){}
    var logoClub=null;try{logoClub=await ppLoadImg(clubData&&clubData.logo_url)}catch(e){}
    // Pre-cargar fotos de mis jugadores implicados (una sola vez)
    var fotosPropias={};
    var idsUsados={};jug.forEach(function(j){(j.vigilantes||[]).forEach(function(id){idsUsados[id]=true})});
    var propios=ppVigilantesInfo(Object.keys(idsUsados));
    for(var k=0;k<propios.length;k++){if(propios[k].photo){try{fotosPropias[propios[k].id]=await ppLoadImg(propios[k].photo)}catch(e){}}}
    var doc=new jspdf.jsPDF('p','mm','a4');
    var W=210,H=297,MG=14,CW=W-MG*2;
    var cDark=[15,23,42],cAccent=[245,158,11],cText=[30,41,59],cGray=[100,116,139];
    // Orden: por lineas (porteros, defensas, medios, delanteros) y luego dorsal
    var ordenPos=[];Object.keys(PP_LINEAS).forEach(function(k){ordenPos=ordenPos.concat(PP_LINEAS[k].posiciones)});
    jug.sort(function(a,b){var pa=ordenPos.indexOf(a.position),pb=ordenPos.indexOf(b.position);if(pa<0)pa=99;if(pb<0)pb=99;if(pa!==pb)return pa-pb;return (a.number||99)-(b.number||99)});
    jug.forEach(function(j,idx){
        if(idx>0)doc.addPage();
        // Cabecera
        doc.setFillColor(cDark[0],cDark[1],cDark[2]);doc.rect(0,0,W,26,'F');
        doc.setFillColor(cAccent[0],cAccent[1],cAccent[2]);doc.rect(0,26,W,1.5,'F');
        if(logoRival){try{doc.addImage(logoRival,'PNG',MG,4,18,18)}catch(e){}}
        doc.setTextColor(255,255,255);doc.setFontSize(16);doc.setFont('helvetica','bold');
        doc.text(('FICHA RIVAL — '+rival).toUpperCase(),logoRival?MG+22:MG,12);
        doc.setFontSize(9);doc.setFont('helvetica','normal');doc.setTextColor(203,213,225);
        doc.text(fecha,logoRival?MG+22:MG,19);
        if(logoClub){try{doc.addImage(logoClub,'PNG',W-MG-18,4,18,18)}catch(e){}}
        // Bloque jugador
        var y=36;
        var fotoW=42;
        doc.setFillColor(241,245,249);doc.roundedRect(MG,y,fotoW,fotoW,4,4,'F');
        if(j.photo){try{doc.addImage(j.photo,'JPEG',MG+1,y+1,fotoW-2,fotoW-2)}catch(e){}}
        else{doc.setTextColor(cAccent[0],cAccent[1],cAccent[2]);doc.setFontSize(30);doc.setFont('helvetica','bold');doc.text(String(j.number||'?'),MG+fotoW/2,y+fotoW/2+10,{align:'center'})}
        var tx=MG+fotoW+8;
        doc.setTextColor(cText[0],cText[1],cText[2]);doc.setFontSize(24);doc.setFont('helvetica','bold');
        doc.text((j.number?j.number+'  ':'')+(j.name||'?'),tx,y+11);
        doc.setFontSize(12);doc.setFont('helvetica','normal');doc.setTextColor(cGray[0],cGray[1],cGray[2]);
        var meta=[];if(j.position)meta.push(j.position);if(j.foot)meta.push(j.foot);if(j.year)meta.push(String(j.year));if(j.club_from)meta.push('Procede: '+j.club_from);
        doc.text(meta.join('  ·  '),tx,y+20);
        var stats=[];if(j.games)stats.push('PJ: '+j.games);if(j.minutes)stats.push('Min: '+j.minutes);if(j.goals)stats.push('Goles: '+j.goals);
        if(stats.length){doc.setFontSize(12);doc.setFont('helvetica','bold');doc.setTextColor(cAccent[0],cAccent[1],cAccent[2]);doc.text(stats.join('   '),tx,y+30)}
        y+=fotoW+12;
        // Analisis
        doc.setFillColor(cDark[0],cDark[1],cDark[2]);doc.rect(MG,y,CW,8,'F');
        doc.setTextColor(255,255,255);doc.setFontSize(11);doc.setFont('helvetica','bold');doc.text('ANÁLISIS',MG+4,y+5.5);
        y+=13;
        doc.setTextColor(cText[0],cText[1],cText[2]);doc.setFontSize(13);doc.setFont('helvetica','normal');
        var lines=doc.splitTextToSize(j.analysis&&j.analysis.trim()?j.analysis.trim():'Sin analisis registrado.',CW-8);
        lines.forEach(function(ln){doc.text(ln,MG+4,y);y+=6.5});
        y+=8;
        // Mis jugadores implicados
        var v=ppVigilantesInfo(j.vigilantes||[]);
        doc.setFillColor(cAccent[0],cAccent[1],cAccent[2]);doc.rect(MG,y,CW,8,'F');
        doc.setTextColor(cDark[0],cDark[1],cDark[2]);doc.setFontSize(11);doc.setFont('helvetica','bold');doc.text('JUGADORES DE '+miEq.toUpperCase()+' IMPLICADOS EN SU VIGILANCIA',MG+4,y+5.5);
        y+=14;
        if(!v.length){doc.setTextColor(cGray[0],cGray[1],cGray[2]);doc.setFontSize(12);doc.setFont('helvetica','italic');doc.text('Sin jugadores asignados.',MG+4,y)}
        else{
            v.forEach(function(p){
                if(y>H-30){doc.addPage();y=20}
                var f=fotosPropias[p.id];
                doc.setFillColor(254,243,199);doc.roundedRect(MG,y-6,CW,16,3,3,'F');
                if(f){try{doc.addImage(f,'PNG',MG+3,y-4.5,13,13)}catch(e){f=null}}
                if(!f){doc.setFillColor(cDark[0],cDark[1],cDark[2]);doc.circle(MG+9.5,y+2,6,'F');doc.setTextColor(cAccent[0],cAccent[1],cAccent[2]);doc.setFontSize(11);doc.setFont('helvetica','bold');doc.text(String(p.number||'?'),MG+9.5,y+3.5,{align:'center'})}
                doc.setTextColor(cText[0],cText[1],cText[2]);doc.setFontSize(14);doc.setFont('helvetica','bold');
                doc.text((p.number?p.number+'  ':'')+p.name,MG+20,y+3.5);
                if(p.position){doc.setFontSize(11);doc.setFont('helvetica','normal');doc.setTextColor(cGray[0],cGray[1],cGray[2]);doc.text(p.position,W-MG-4,y+3.5,{align:'right'})}
                y+=20;
            });
        }
        // Pie
        doc.setDrawColor(226,232,240);doc.setLineWidth(0.3);doc.line(MG,H-14,W-MG,H-14);
        doc.setFontSize(8);doc.setFont('helvetica','normal');doc.setTextColor(cGray[0],cGray[1],cGray[2]);
        doc.text(miEq+' · Plan de partido vs '+rival,MG,H-9);
        doc.text('Ficha '+(idx+1)+' de '+jug.length,W-MG,H-9,{align:'right'});
    });
    doc.save('fichas_vestuario_'+rival.replace(/[^a-z0-9]/gi,'_')+'.pdf');
}
// =============================================
// FASES PERSONALIZABLES (renombrar, añadir, eliminar, guardar por defecto en el club)
// =============================================
async function ppGuardarFasesArray(f){try{await supabaseClient.from('match_plans').update({tactical_phases:f,updated_at:new Date().toISOString()}).eq('id',pp.planActual.id);pp.planActual.tactical_phases=f;return true}catch(e){showToast('Error: '+e.message);return false}}
async function ppRenombrarFase(i){var f=ppGetFases();var nuevo=prompt('Nuevo nombre de la fase:',f[i].title);if(nuevo===null)return;nuevo=nuevo.trim();if(!nuevo){showToast('El nombre no puede estar vacio');return}f[i].title=nuevo;if(await ppGuardarFasesArray(f))ppMostrarTab('fases')}
async function ppEliminarFase(i){var f=ppGetFases();if(f[i].notes||(f[i].media&&f[i].media.length)){showToast('Esta fase tiene contenido. Borra primero las notas y los archivos','warning');return}if(f.length<=1){showToast('Debe quedar al menos una fase');return}if(!confirm('¿Eliminar la fase "'+f[i].title+'"?'))return;f.splice(i,1);if(await ppGuardarFasesArray(f))ppMostrarTab('fases')}
