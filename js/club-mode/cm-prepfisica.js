...
h+='<div style="overflow-x:auto"><table style="width:100%;border-collapse:collapse;font-size:13px;color:#e2e8f0"><thead><tr style="background:#1e293b;color:#94a3b8;font-size:11px;text-transform:uppercase"><th style="text-align:left;padding:8px 10px">Jugador</th><th style="padding:8px 6px">Pos.</th><th style="padding:8px 6px" title="Partidos (completos)">PJ</th><th style="padding:8px 6px">Min</th>';
...
h+='<tr style="background:#0f3d3e;font-weight:600"><td style="padding:8px 10px;color:#14b8a6">Media equipo</td><td></td><td></td><td></td>';cols.forEach(function(co){h+='<td style="text-align:center;padding:8px 6px;color:#14b8a6">'+fmt(med[co.k],co.d)+'</td>';});h+='</tr>';
d.forEach(function(x){var proy=!(x.n_reales>0);var st=proy?'font-style:italic;color:#cbd5e1':'';h+='<tr style="border-bottom:1px solid #1e293b;cursor:pointer" onclick="cmPfVerDetallePerfil(\''+x.player_id+'\',\''+String(x.player_name||'').replace(/'/g,'')+'\')"><td style="padding:8px 10px;font-weight:600">'+(x.player_name||'?')+'</td><td style="text-align:center;padding:8px 6px;color:#94a3b8">'+(x.player_position||'')+'</td><td style="text-align:center;padding:8px 6px">'+x.n_partidos+' <span style="color:#64748b">('+x.n_reales+')</span></td><td style="text-align:center;padding:8px 6px;color:#94a3b8">'+fmt(x.min_totales,0)+'</td>';
...
h+='</tbody></table></div><p style="color:#64748b;font-size:11px;margin-top:8px">Verde: &gt;10% sobre la media del equipo &middot; Rojo: &lt;10% bajo la media &middot; PJ: partidos usados (entre parentesis, partidos de 90\' completos). Pulsa un jugador para ver partido a partido.</p>';
c.innerHTML=h;}
