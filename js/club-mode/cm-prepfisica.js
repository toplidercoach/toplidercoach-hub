...
function cmPfRenderGpsTab(ct,datos,acwr){
    ...
    h+='<div id="cmpf-ie-evolucion"></div>';
    // Historial
...
function cmPfCambiarVista(vista) {
    ...
    var vp = document.getElementById('cmpf-vista-perfil');
    if (vp) vp.style.display = vista === 'perfil' ? 'block' : 'none';
    if (vista === 'perfil') cmPfCargarPerfilPartido();
...
// ========== PERFIL DE RENDIMIENTO EN PARTIDO ==========
var cmPfPerfilData=[],cmPfPerfilOrden='total_distance_90';
async function cmPfCargarPerfilPartido(){var c=document.getElementById('cmpf-perfil-contenido');if(!c)return;c.innerHTML='<div class="cmpf-empty"><div class="icon">...</div><p>Calculando perfiles...</p></div>';try{var r=await supabaseClient.from('cm_pf_perfil_partido_nombres').select('*').eq('club_id',clubId);if(r.error)throw r.error;cmPfPerfilData=r.data||[];cmPfRenderPerfilPartido();}catch(e){console.error(e);c.innerHTML='<div class="cmpf-empty"><p>Error cargando el perfil: '+(e.message||e)+'</p></div>';}}
function cmPfOrdenarPerfil(k){cmPfPerfilOrden=k;cmPfRenderPerfilPartido();}
function cmPfRenderPerfilPartido(){var c=document.getElementById('cmpf-perfil-contenido');if(!c)return;var d=cmPfPerfilData.slice();if(!d.length){c.innerHTML='<div class="cmpf-empty"><p>Aun no hay partidos con GPS que casen con el calendario de Partidos.</p></div>';return;}
var cols=[{k:'total_distance_90',l:'Dist. (m)',d:0},{k:'hsr_90',l:'HSR (m)',d:0},{k:'sprint_90',l:'Sprint (m)',d:0},{k:'sprint_count_90',l:'N. sprints',d:1},{k:'accel_90',l:'Acel.',d:1},{k:'decel_90',l:'Desacel.',d:1},{k:'player_load_90',l:'P. Load',d:1},{k:'max_speed_kmh',l:'Vmax',d:1}];
d.sort(function(a,b){return (parseFloat(b[cmPfPerfilOrden])||0)-(parseFloat(a[cmPfPerfilOrden])||0);});
var med={};cols.forEach(function(co){var v=d.map(function(x){return parseFloat(x[co.k]);}).filter(function(x){return !isNaN(x);});med[co.k]=v.length?v.reduce(function(a,b){return a+b;},0)/v.length:null;});
var fc=cmPfGpsConfig&&cmPfGpsConfig.fatigue_coef?cmPfGpsConfig.fatigue_coef:{};var fmt=function(v,dec){if(v==null||isNaN(parseFloat(v)))return'-';return parseFloat(v).toLocaleString('es-ES',{minimumFractionDigits:dec,maximumFractionDigits:dec});};
var nReal=d.filter(function(x){return x.n_reales>0;}).length;var nPart=Math.max.apply(null,d.map(function(x){return x.n_partidos;}));
var h='<div class="cmpf-stats-bar"><div class="cmpf-stat"><div class="num">'+d.length+'</div><div class="label">Jugadores con perfil</div></div><div class="cmpf-stat"><div class="num">'+nReal+'</div><div class="label">Con al menos un 90\' real</div></div><div class="cmpf-stat"><div class="num">'+nPart+'</div><div class="label">Partidos (max.)</div></div><div class="cmpf-stat"><div class="num">'+fmt(med.total_distance_90,0)+'</div><div class="label">Media equipo dist. 90\'</div></div></div>';
h+='<p style="color:#94a3b8;font-size:12px;margin:0 0 12px 0">Valores proyectados a 90\' y ponderados por minutos. Solo cuentan partidos del calendario, segmento TOTAL y &ge; '+((cmPfGpsConfig&&cmPfGpsConfig.perfil_min_minutes)||30)+'\'. <em>En cursiva con &asymp;</em>: perfil basado solo en proyecciones (ningun partido completo). Coef. 2&ordf; parte: dist '+(fc.total||'-')+' &middot; HSR '+(fc.hsr||'-')+' &middot; sprint '+(fc.sprint||'-')+'. Pulsa una columna para ordenar.</p>';
h+='<div style="overflow-x:auto"><table style="width:100%;border-collapse:collapse;font-size:13px;color:#e2e8f0"><thead><tr style="background:#1e293b;color:#94a3b8;font-size:11px;text-transform:uppercase"><th style="text-align:left;padding:8px 10px">Jugador</th><th style="padding:8px 6px">Pos.</th><th style="padding:8px 6px" title="Fiabilidad del perfil">Fiab.</th><th style="padding:8px 6px" title="Partidos (completos)">PJ</th><th style="padding:8px 6px">Min</th>';
cols.forEach(function(co){var act=co.k===cmPfPerfilOrden;h+='<th style="padding:8px 6px;cursor:pointer;white-space:nowrap;color:'+(act?'#14b8a6':'#94a3b8')+'" onclick="cmPfOrdenarPerfil(\''+co.k+'\')">'+co.l+(act?' &#9660;':'')+'</th>';});h+='</tr></thead><tbody>';
h+='<tr style="background:#0f3d3e;font-weight:600"><td style="padding:8px 10px;color:#14b8a6">Media equipo</td><td></td><td></td><td></td><td></td>';cols.forEach(function(co){h+='<td style="text-align:center;padding:8px 6px;color:#14b8a6">'+fmt(med[co.k],co.d)+'</td>';});h+='</tr>';
d.forEach(function(x){var pct=parseFloat(x.pct_real)||0;var nivel=x.n_reales>=2||pct>=60?2:(x.n_reales>0?1:0);var proy=nivel===0;var st=proy?'font-style:italic;color:#cbd5e1':'';var fiab=nivel===2?'<span title="Solido: '+pct+'% de minutos reales" style="color:#22c55e">&#9679;</span>':nivel===1?'<span title="Mezcla: '+pct+'% de minutos reales" style="color:#f59e0b">&#9679;</span>':'<span title="Solo proyecciones" style="color:#64748b">&#9679;</span>';h+='<tr style="border-bottom:1px solid #1e293b;cursor:pointer" onclick="cmPfVerDetallePerfil(\''+x.player_id+'\',\''+String(x.player_name||'').replace(/'/g,'')+'\')"><td style="padding:8px 10px;font-weight:600">'+(x.player_name||'?')+'</td><td style="text-align:center;padding:8px 6px;color:#94a3b8">'+(x.player_position||'')+'</td><td style="text-align:center;padding:8px 6px">'+fiab+'</td><td style="text-align:center;padding:8px 6px">'+x.n_partidos+' <span style="color:#64748b">('+x.n_reales+')</span></td><td style="text-align:center;padding:8px 6px;color:#94a3b8">'+fmt(x.min_totales,0)+'</td>';
cols.forEach(function(co){var v=x[co.k];var dif=med[co.k]&&v!=null?((parseFloat(v)-med[co.k])/med[co.k]*100):null;var col=dif==null?'':(dif>10?'#22c55e':dif<-10?'#ef4444':'#e2e8f0');h+='<td style="text-align:center;padding:8px 6px;'+st+(col?';color:'+col:'')+'">'+(proy&&co.k!=='max_speed_kmh'?'&asymp; ':'')+fmt(v,co.d)+'</td>';});h+='</tr>';});
h+='</tbody></table></div><p style="color:#64748b;font-size:11px;margin-top:8px">Fiab.: <span style="color:#22c55e">&#9679;</span> solido (&ge;2 partidos completos o &ge;60% min reales) &middot; <span style="color:#f59e0b">&#9679;</span> mezcla &middot; <span style="color:#64748b">&#9679;</span> solo proyecciones &middot; Verde: &gt;10% sobre la media del equipo &middot; Rojo: &lt;10% bajo la media &middot; PJ: partidos usados (entre parentesis, partidos de 90\' completos). Pulsa un jugador para ver partido a partido.</p>';
c.innerHTML=h;}
async function cmPfVerDetallePerfil(pid,nombre){try{var r=await supabaseClient.from('cm_pf_perfil_partido_detalle').select('*').eq('club_id',clubId).eq('player_id',pid).order('session_date',{ascending:false});var d=r.data||[];if(!d.length){showToast('Sin partidos');return;}var fmt=function(v,dec){if(v==null)return'-';return parseFloat(v).toLocaleString('es-ES',{minimumFractionDigits:dec,maximumFractionDigits:dec});};var rows='';d.forEach(function(x){var st=x.es_real?'':'font-style:italic;color:#cbd5e1';var pre=x.es_real?'':'&asymp; ';rows+='<tr style="border-bottom:1px solid #1e293b"><td style="padding:6px 8px">'+cmPfFormatFecha(x.session_date)+'</td><td style="padding:6px 8px">'+(x.opponent||'-')+'</td><td style="text-align:center;padding:6px 8px">'+fmt(x.duration_min,0)+'\''+(x.es_titular?'':' <span style="color:#64748b" title="Suplente">S</span>')+'</td><td style="text-align:center;padding:6px 8px;'+st+'">'+pre+fmt(x.total_distance_90,0)+'</td><td style="text-align:center;padding:6px 8px;'+st+'">'+pre+fmt(x.hsr_90,0)+'</td><td style="text-align:center;padding:6px 8px;'+st+'">'+pre+fmt(x.sprint_90,0)+'</td><td style="text-align:center;padding:6px 8px;'+st+'">'+pre+fmt(x.accel_90,1)+'</td><td style="text-align:center;padding:6px 8px;'+st+'">'+pre+fmt(x.decel_90,1)+'</td><td style="text-align:center;padding:6px 8px;'+st+'">'+pre+fmt(x.player_load_90,1)+'</td><td style="text-align:center;padding:6px 8px">'+fmt(x.max_speed_kmh,1)+'</td></tr>';});
var ov=document.createElement('div');ov.style.cssText='position:fixed;top:0;left:0;width:100%;height:100%;background:rgba(0,0,0,.65);z-index:9500;display:flex;justify-content:center;align-items:center;padding:20px';ov.onclick=function(e){if(e.target===ov)document.body.removeChild(ov);};ov.innerHTML='<div style="background:#0f172a;border:1px solid #14b8a6;border-radius:12px;padding:20px;max-width:900px;width:100%;max-height:80vh;overflow:auto"><div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:12px"><h4 style="margin:0;color:#e2e8f0">'+nombre+' &middot; partido a partido (proyectado a 90\')</h4><button style="background:none;border:none;color:#94a3b8;font-size:22px;cursor:pointer" onclick="this.closest(\'div[style*=fixed]\').remove()">&times;</button></div><table style="width:100%;border-collapse:collapse;font-size:12px;color:#e2e8f0"><thead><tr style="background:#1e293b;color:#94a3b8;font-size:11px;text-transform:uppercase"><th style="text-align:left;padding:6px 8px">Fecha</th><th style="text-align:left;padding:6px 8px">Rival</th><th style="padding:6px 8px">Min</th><th style="padding:6px 8px">Dist.</th><th style="padding:6px 8px">HSR</th><th style="padding:6px 8px">Sprint</th><th style="padding:6px 8px">Acel.</th><th style="padding:6px 8px">Desac.</th><th style="padding:6px 8px">P.Load</th><th style="padding:6px 8px">Vmax</th></tr></thead><tbody>'+rows+'</tbody></table><p style="color:#64748b;font-size:11px;margin:10px 0 0 0">&asymp; = proyectado a 90\' &middot; S = entro como suplente (tramo final del partido)</p></div>';document.body.appendChild(ov);}catch(e){showToast('Error: '+(e.message||e));}}
